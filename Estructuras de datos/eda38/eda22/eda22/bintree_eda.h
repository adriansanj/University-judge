...
public:
	...
	//Devuelve número de hojas
	int numHojas() const {
		if (empty()) {
			return 0;
		}
		else {
			if (raiz->left == nullptr && raiz->right == nullptr) { //Si no tiene hijos, nos encontramos con que se trata de una hoja
				return 1;
			}
			else if (raiz->left != nullptr && raiz->right != nullptr) { //Comprobamos que tiene dos hijos para que no salte una excepción
				return (left().numHojas() + right().numHojas());
			}
			//Vemos cuál de los dos hijos no es vacío
			else if (raiz->left != nullptr) {
				return left().numHojas();
			}
			else {
				return right().numHojas();
			}
		}
	}
	...
	int altura() const {
		...
		if (raiz->right != nullptr) {
			maxim = std::max(right().altura(), maxim);
		}
		if (raiz->left != nullptr) {
			maxim = std::max(left().altura(), maxim);
		}

		return maxim + 1;
	}

	//Devuelve la frontera
	...
	//Devuelve minimo suponiendo que el tipo de dato es comparable con <
	T minimo() {
		if (!empty()) {
			if (raiz->left == nullptr && raiz->right == nullptr) {
				...
				T aux;
				aux = std::min(left().minimo(), right().minimo());
				return std::min(raiz->elem, aux);
			}
			else if (raiz->left != nullptr) {
				return std::min(raiz->elem, left().minimo());
			}
			else {
				return std::min(raiz->elem, right().minimo());
			}
		}
...
protected:
	/*void minimo(Link a, T & min) const {
		if (a != nullptr) {
			if (a->elem < min) {
				min = a->elem;
			}
	...
	void frontera(Link a, std::vector<T> & front) const {
		if (a != nullptr) {
