...
protected:
	...
	void lowerBound(std::pair<bool, T> &p, T const& e,Link raiz) const {
		if (raiz != nullptr) {
			if (menor(e, raiz->elem) || e == raiz->elem) {
				p.first = true;
				p.second = raiz->elem;
				lowerBound(p,e, raiz->iz);
			}
			else if (menor(raiz->elem, e)) {
				lowerBound(p,e, raiz->dr);
			}
		}
